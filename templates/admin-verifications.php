    ...
    foreach ( $sellers as $seller ) {
       ...
       if ( isset($seller_profile['dokan_verification']['info']['dokan_v_id_status']) && $seller_profile['dokan_verification']['info']['dokan_v_id_status'] === $status ) {
        ...
        if ( isset( $seller_profile['dokan_verification']['info']['store_address'] ) && $seller_profile['dokan_verification']['info']['store_address']['v_status'] === $status ) {
            ...
            $seller_v['address_status'] = isset( 
                $seller_profile['dokan_verification']['info']['store_address']['v_status'] ) ? $seller_profile['dokan_verification']['info']['store_address']['v_status'] : '';
        }
        ...
        if ( isset( $seller_profile['dokan_verification']['info']['seller_ein'] ) && $seller_profile['dokan_verification']['info']['dokan_v_ein_status'] === $status ) {
            $seller_v['seller_ein'] = $seller_profile['dokan_verification']['info']['seller_ein'];
            $seller_v['seller_ein_status'] = $seller_profile['dokan_verification']['info']['dokan_v_ein_status'];
            echo $seller_v['seller_ein'];
           
        }
        else {
            $seller_v['seller_ein'] =  isset( $seller_profile['dokan_verification']['info']['seller_ein'] ) ? $seller_profile['dokan_verification']['info']['seller_ein'] : '';
            $seller_v['seller_ein_status'] = isset($seller_profile['dokan_verification']['info']['dokan_v_ein_status'])?$seller_profile['dokan_verification']['info']['dokan_v_ein_status']:'';
        }
    }
    if ( isset( $seller_v ) ) {
        $result[] = $seller_v;
...
<!--                        <input type="checkbox" class="dokan-withdraw-allcheck">-->
                    ...
                    <th width="20%"><?php _e( 'Store Name', 'dokan' ); ?></th>
                    <th width="20%"><?php _e( 'Photo ID', 'dokan' ); ?></th>
                    <th width="20%"><?php _e( 'Address', 'dokan' ); ?></th>
                    <th width="20%"><?php _e( 'Phone Number', 'dokan' ); ?></th>
                    <th width="20%"><?php _e( 'Seller EIN', 'dokan' ); ?></th>
                </tr>
            </thead>
            <tfoot>
                <tr>
...
<!--                            <input type="checkbox" name="id[<?php echo $val['seller_id']; ?>]" value="<?php echo $val['seller_id']; ?>">-->
                        ...
                        <td>
                            <?php if( isset($val['seller_ein']) ){ ?>
                            <form method="post" action="" class="dokan-admin-sv-action">
                                 <?php wp_nonce_field( 'dokan_sv_nonce_action', 'dokan_sv_nonce' ); ?>
                                <div class="d_v_address">
                                    <p><?php echo $val['seller_ein'];  ?></p>
                                    <input type="hidden" class="dokan-file-field" value="<?php echo $val['seller_ein']; ?>" name="seller_ein">

                                </div>
                                <div class="row-actions">
                                    <?php if ( $status == 'pending' ) { ?>
                                        <span class="edit"><a href="#" class="dokan-sv-action" data-type="ein" data-status="approved" data-seller_id = "<?php echo $val['seller_id']; ?>"><?php _e( 'Approve', 'dokan' ); ?></a> | </span>
                                        <span class="trash"><a href="#" class="dokan-sv-action" data-type="ein" data-status="rejected" data-seller_id = "<?php echo $val['seller_id']; ?>"><?php _e( 'Reject', 'dokan' ); ?></a></span>

                                    <?php } elseif ( $status == 'approved' ) { ?>

                                        <span class="trash"><a href="#" class="dokan-sv-action" data-type="ein" data-status="disapproved" data-seller_id = "<?php echo $val['seller_id']; ?>"><?php _e( 'Disapprove', 'dokan' ); ?></a> | </span>

                                    <?php } elseif ( $status == 'rejected' ) { ?>

                                        <span class="edit"><a href="#" class="dokan-sv-action" data-type="ein" data-status="approved" data-seller_id = "<?php echo $val['seller_id']; ?>"><?php _e( 'Approve', 'dokan' ); ?></a> | </span>
                                        <span class="edit"><a href="#" class="dokan-sv-action" data-type="ein" data-status="pending" data-seller_id = "<?php echo $val['seller_id']; ?>"><?php _e( 'Pending', 'dokan' ); ?></a></span>
