...
if ( !isset( $seller_profile['seller_ein'] ) ) {
    $seller_profile['seller_ein']      = '';
}
else {
    $einnumber= $seller_profile['seller_ein'];
}
...
$einnumber      = isset($seller_profile['dokan_verification']['info']['seller_ein'] ) ? $seller_profile['dokan_verification']['info']['seller_ein'] : '';
?>
...
<div class="dokan-verification-content">
    ...
    <div class="dokan-panel dokan-panel-default dokan_v_id" id="dokan_v_ein">
        <div class="dokan-panel-heading">
            <strong><?php _e( 'EIN Verification', 'dokan' ); ?></strong>
        </div>
        ...
        <div class="dokan-panel-body">
            ...
            if ( $ein_status != '' ) {
                ...
                ?>
                <div class="dokan-alert <?php echo $alert_class ?>" id="dokan_v_ein_feedback">
                    <?php echo sprintf( __( 'Your EIN verification request is %s', 'dokan' ), $ein_status ); ?>
                </div>
            <?php } ?>
            ...
            if ( $ein_status == 'rejected' || $ein_status == '' )
                $ein_btn_class = '';
            else {
                $ein_btn_class = 'dokan-hide';
            }
            ?>

            <button class="dokan-btn dokan-btn-theme dokan-v-start-btn <?php echo esc_attr($ein_btn_class)?>" id="dokan_v_ein_click"><?php _e( 'Start Verification', 'dokan' ); ?></button>


            <div class="dokan_v_ein_info_box dokan-hide">
                    <form method="post" id="dokan-verify-ein-form"  action="" class="dokan-form-horizontal">
                        <h4><?php _e( 'EIN Number :', 'dokan' ); ?></h4>

                        <div class="document-box">
                          
                            <label> <?php _e('EIN Number','dokan'); ?>
                            <input id="einnumber" value="<?php echo $einnumber; ?>" name="einnumber" placeholder="Ein Number" class="dokan-form-control input-md" type="text">
                            <div class="dokan-form-group dokan-verify-ein">
                                <div class="dokan-w5 dokan-gravatar">
                                     <?php wp_nonce_field( 'dokan_verify_ein', 'dokan_verify_ein_nonce' ); ?>
                                    <input type="submit" id='dokan_v_ein_submit' class="dokan-btn dokan-btn-theme" value="<?php esc_attr_e( 'Submit', 'dokan' ); ?>">
                                    <input type="button" id='dokan_v_ein_cancel_form' class="dokan-btn dokan-btn-theme dokan-v-cancel-btn" value="<?php esc_attr_e( 'Cancel', 'dokan' ); ?>">
            ...
            <?php
            if ( !$ein_status == 'pending' ) {
                $cancel_btn_class = 'dokan-hide';
            }
            ?>
            <button class="dokan-btn dokan-btn-theme <?php echo esc_attr($cancel_btn_class); ?>" id="dokan_v_ein_cancel"><?php _e( 'Cancel Request', 'dokan' ); ?></button>
        </div>
    </div>
            ...
            <div class="dokan_v_id_info_box dokan-hide">
                    <form method="post" id="dokan-verify-id-form"  action="" class="dokan-form-horizontal">
                        ...
                        <div class="document-box">
                            ...
                            <label> <?php _e('EIN Number','dokan'); ?>
                            <input id="einnumber1" value="<?php echo $einnumber; ?>" name="einnumber" placeholder="Ein Number" class="dokan-form-control input-md" type="text">
