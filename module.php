...
class Module {
    ...
    public function init_hooks() {
        ...
        add_action('wp_ajax_dokan_update_verify_ein', array( $this, 'dokan_update_verify_ein' ));
        add_action( 'wp_ajax_dokan_ein_verification_cancel', array( $this, 'dokan_ein_verification_cancel' ) );
    ...
    **/
    ...
    public function dokan_update_verify_info() {
        ...
        if ( isset( $postdata['dokan_v_id_type'] ) && isset( $postdata['dokan_gravatar'] ) ) {
            ...
            $seller_profile['dokan_verification']['info']['seller_ein'] = $postdata['einnumber'];
    ...
    /** Updates EIN Number for verification
     *
     * @since 1.0.0
     * @return void
     */
    public function dokan_update_verify_ein() {
        parse_str( $_POST['data'], $postdata );

        if ( !wp_verify_nonce( $postdata['dokan_verify_ein_nonce'], 'dokan_verify_ein' ) ) {
            wp_send_json_error( __( 'Are you cheating?', 'dokan' ) );
        }
        ...
        $seller_profile = dokan_get_store_info( $user_id );
            ...
            $seller_profile['dokan_verification']['info']['dokan_v_ein_status'] = 'pending';
            $seller_profile['dokan_verification']['info']['seller_ein'] = $postdata['einnumber'];
            ?>
                
            <?php
            $msg = sprintf( __( 'Your EIN verification request is sent and %s approval', 'dokan' ), $seller_profile['dokan_verification']['info']['dokan_seller_ein_status'] );
            dokan_verification_request_submit_email();
            
        update_user_meta($user_id,'dokan_verification_status','pending');
    ...
    public function dokan_id_verification_cancel() {
      ...
      //  unset($seller_profile['dokan_verification']['info']['seller_ein']);
      //  unset($seller_profile['dokan_verification']['info']['dokan_v_ein_status']);
    ...
    public function dokan_ein_verification_cancel(){
        // $user_id        = get_current_user_id();
        // $seller_profile = dokan_get_store_info( $user_id );
        // unset($seller_profile['dokan_verification']['info']['seller_ein']);
        // unset($seller_profile['dokan_verification']['info']['dokan_v_ein_status']);
        ...
        // update_user_meta( $user_id,'dokan_profile_setting',$seller_profile);
        
        // $msg = __( 'Your EIN Verification request is cancelled', 'dokan' );
    ...
    public function dokan_sv_form_action() {
        ...
        switch ( $postdata['status'] ) {
            case 'approved':
                ...
                elseif($postdata['type'] == 'ein'){

                    $seller_profile['dokan_verification']['varified_info']['seller_ein'] = $postdata['seller_ein'];
                    update_user_meta($user_id,'dokan_ein',$seller_profile['dokan_verification']['info']['seller_ein']);

                    $seller_profile['dokan_verification']['info']['dokan_v_ein_status'] ='approved';
            ...
            case 'rejected' :
                ...
                elseif($postdata['type'] === 'ein'){
                    $seller_profile['dokan_verification']['info']['dokan_v_ein_status'] = 'rejected';
            ...
            case 'disapproved' :
                ...
                elseif($postdata['type'] === 'ein'){
                    unset($seller_profile['dokan_verification']['varified_info']['seller_ein']);
                    $seller_profile['dokan_verification']['info']['dokan_v_ein_status'] = 'pending';
                    update_user_meta($user_id,'dokan_ein',$seller_profile['dokan_verification']['info']['seller_ein']);
    ...
    public function dokan_v_recheck_verification_status_meta( $meta_id, $object_id, $meta_key, $_meta_value ) {
        ...
        $ein_status = isset($seller_profile['dokan_verification']['info']['dokan_v_ein_status'])? $seller_profile['dokan_verification']['info']['dokan_v_ein_status']:'';

        if ( $id_status === $address_status || $id_status === $ein_status ) {
            update_user_meta( $current_user, 'dokan_verification_status', $id_status );
        } elseif ( ($id_status === 'pending' || $id_status === 'approved' || $id_status === 'rejected' ) && ( $address_status === 'approved' || $address_status === 'pending' || $address_status === 'rejected' ) && ( $ein_status === 'approved' || $ein_status === 'pending' || $ein_status === 'rejected' )) {
            $st = $id_status . ',' . $address_status .','. $ein_status;
            update_user_meta( $current_user, 'dokan_verification_status', $st );
        } elseif ( ($id_status === 'pending' || $id_status === 'approved' || $id_status === 'rejected') && $address_status === '' && $ein_status === '') {
            update_user_meta( $current_user, 'dokan_verification_status', $id_status );
        } elseif ( $id_status === '' && ( $address_status === 'approved' || $address_status === 'pending' || $address_status === 'rejected' ) && $ein_status === '') {
            update_user_meta( $current_user, 'dokan_verification_status', $address_status );
        }
         elseif ( ($ein_status === 'pending' || $ein_status === 'approved' || $ein_status === 'rejected') && $address_status === ''&& $id_status === '') {
            update_user_meta( $current_user, 'dokan_verification_status', $ein_status );
        }
