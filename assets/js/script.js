...
( function ( $ ) {
    ...
    //ID verification
        ...
        //EIN verification
        // show verification panel on start click
        $( 'button#dokan_v_ein_click' ).click( function () {
            $(this).slideUp( 'fast',function(){
                $( '.dokan_v_ein_info_box' ).slideDown('fast');
            });
        } );
        
        // close verification panel on cancel click
        $( 'input#dokan_v_ein_cancel_form' ).click( function () {
            $( '.dokan_v_ein_info_box' ).slideUp( 'fast',function(){
                $( 'button#dokan_v_ein_click' ).slideDown('fast');
            });
        } );
        ...
        $( 'button#dokan_v_id_cancel' ).click( function () {
           ...
           // submit EIN verification request
           $( '.dokan-verification-content' ).on( 'click', 'input#dokan_v_ein_submit', function ( e ) {
            e.preventDefault();

            var self = $( this ),
                data = {
                    action: 'dokan_update_verify_ein',
                    data: self.closest( '#dokan-verify-ein-form' ).serialize(),
                };

            feedback.fadeOut();
            var regex = /^\d{2}\-?\d{7}$/;
            if(regex.test($('#einnumber').val())){
                $.post( dokan.ajaxurl, data, function ( resp ) {
                    if ( resp.success == true ) {
                        $( '#dokan_v_ein_feedback' ).addClass( 'hidden' );
                        feedback.addClass( 'dokan-alert dokan-alert-success' );
                        feedback.html( resp.data );
                        feedback.fadeIn();
                        console.log('lets see');
                        $( 'div.dokan_v_ein_info_box' ).hide();
                        $( 'button#dokan_v_ein_cancel' ).show();                    
                    } else {
                        feedback.addClass( 'dokan-alert dokan-alert-danger' );
                        feedback.html( 'failed' );
                        feedback.fadeIn();
                ...
                feedback.addClass( 'dokan-alert dokan-alert-danger' );
                feedback.html( 'please check the format of EIN number' );
                feedback.fadeIn();
            }
        ...
        // cancel Verification EIN request
       
         $( 'button#dokan_v_ein_cancel' ).click( function () {
            data = {
                action: 'dokan_ein_verification_cancel',
                data: 'cancel',
            };
            ...
            $.post( dokan.ajaxurl, data, function ( resp ) {
                if ( resp.success == true ) {
                    ...
                    feedback.html( resp.data );
                    feedback.fadeIn();
                    $( 'button#dokan_v_ein_cancel' ).hide();
                    $( 'button#dokan_v_ein_click' ).removeClass('dokan-hide');
                    $( 'button#dokan_v_ein_click' ).show();
